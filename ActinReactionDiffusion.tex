...
\subsection{Validating stochastic simulations with ODEs}
The algorithm in Section\ \ref{sec:SSA} is a stochastic version of the following ODEs \cite{edelstein1998models}
\begin{subequations}
\label{eq:ODEsMix}
\begin{align}
\frac{d \Nm}{dt} &= -2 (\kdp/V) \Nm^2 + 2 \kdm \Nd - (\ktrp/V) \Nm \Nd + \ktrm \Ntr + \sum_{j=4}^M \left(- (\kfp/V) \Nm N_{j-1}+\kfm N_j\right)\\
\frac{d \Nd}{dt} &= (\kdp/V) \Nm^2 - \kdm \Nd -(\ktrp/V) \Nd \Nm + \ktrm \Ntr \\
\frac{d \Ntr}{dt} &= (\ktrp/V) \Nm \Nd - \ktrm \Ntr - (\kfp/V) \Ntr \Nm + \kfm N_4 \\
\frac{d N_j}{dt} & = (\kfp/V) \Nm N_{j-1} - \kfm N_j - (\kfp/V) N_j \Nm + \kfm N_{j+1} \qquad 4 \leq j < M \\
\frac{d N_M}{dt} & = (\kfp/V) \Nm N_{M-1} - \kfm N_M
\end{align}
\end{subequations}
...
\item This fiber can add monomers to the barbed and pointed ends, with an accelerated rate at the barbed end $\alpha_\text{for}\kbp$. It can also depolymerize from both ends with the normal rates, except that we do not allow formin nucleates (2 monomers + formin) to break apart.
