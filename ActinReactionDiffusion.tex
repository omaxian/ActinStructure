...
\end{enumerate}
In the language of our simulation platform, the rates with formin are
\begin{gather*}
\nonumber
\kdp_{ij} = \kdp_0 \begin{pmatrix} 1 & 0.057 \, \textrm{nM}^{-1}\end{pmatrix} \qquad 
\ktrp_{ij} =\ktrp_0 \begin{pmatrix} 1 & 5.5 \times 10^4 \end{pmatrix} \qquad 
\kbp_{ij} =\kbp_0 \begin{pmatrix} 1 & 0.5\end{pmatrix} \\  \kon_1 = 29.1 \, \mu \textrm{M}^{-1} \cdot\text{s}^{-1}\qquad \koff_1 = 5 \times 10^{-4} \, \text{s}^{-1}  \\ \nonumber
\kdm_j = \kdm_0 \begin{pmatrix} 1 & 0 \end{pmatrix} \qquad 
\ktrm_j = \ktrm_0 \begin{pmatrix} 1 & 0 \end{pmatrix}\qquad 
\kbm = 1.4 \, \text{s}^{-1}\begin{pmatrix} 1 & 1 \end{pmatrix}
\end{gather*}
Note the large rate for the trimers; we assume that formin-bound dimers become trimers with rate $\afor \kbp_0+\kpp_0$, which is divided by $\ktrp_0$ to obtain the proportion. We assume that formin bound dimers and trimers do not depolymerize.
...
The reaction of profilin with actin is known to have equilibrium constant 4.5 $\mu$M$^{-1}$ \cite{perelroizen1994interaction}, which means that profilin aggressively binds actin monomers. We work off the fact that profilin inhibits spontaneous nucleation \cite{paul2009review}, and average the rates reported in \cite{kovar2006control} (for mDia1 and mDia2) to obtain a rate for how profilin affects assembly at the barbed end. This gives the rates
\begin{gather*}
\nonumber
