...
void rotateVector(const vec3 &Omega, vec3 &input){
    double nOmTol = 1e-10;
    double nOm = sqrt(dot(Omega,Omega));
    if (nOm > nOmTol){
        vec3 OmegaHat;
        for (int d=0; d < 3; d++){
            OmegaHat[d]=Omega[d]/nOm;
        }
        double OmegaDotTau = dot(OmegaHat,input);
        vec3 OmegaCrossTau;
        cross(OmegaHat,input,OmegaCrossTau);
        for (int d = 0; d < 3; d++){
            input[d] = input[d]*cos(nOm)+OmegaCrossTau[d]*sin(nOm)+OmegaHat[d]*OmegaDotTau*(1.0-cos(nOm));
        }
    } 
}
